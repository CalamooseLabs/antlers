...
% \sigspace is the clear vertical room reserved ABOVE each signature rule so
% there is somewhere to actually sign by hand. Tune it once here to loosen or
% tighten every signature block in every document at the same time. (Leading
% \vspace is discardable, so a block landing at the top of a page isn't shoved
% down by it.)
\newlength{\sigspace}
\setlength{\sigspace}{0.5in}

% A signature rule (default 3in) with an optional caption line beneath it.
\newcommand{\sigline}[1][3in]{\rule{#1}{0.4pt}}
% \sigblock{caption} -> signing room, a signature rule, then a caption beneath
% it. \nobreak keeps the caption glued to its rule across a page break.
\newcommand{\sigblock}[1]{%
	\par\vspace{\sigspace}\noindent\sigline\par\nobreak\vspace{-0.3em}\noindent #1\par
}

% \siggutter is the horizontal gap between the two signature lines that share a
% row in a two-up block, so the left and right signatures don't run together.
\newlength{\siggutter}
\setlength{\siggutter}{0.4in}
\newlength{\sigcol}

% Two-up signature row: two separate signature rules side by side (a \siggutter
% gap between them) with a caption beneath each. Shared by the board/shareholder
% consents, whose signature grid lists the directors or shareholders two per row.
\newcommand{\sigpair}[2]{%
	\setlength{\sigcol}{0.5\textwidth}\addtolength{\sigcol}{-0.5\siggutter}%
	\par\vspace{\sigspace}\noindent%
	\rule{\sigcol}{0.4pt}\hspace{\siggutter}\rule{\sigcol}{0.4pt}\par\nobreak\vspace{0.2em}%
	\noindent\makebox[\sigcol][l]{#1}\hspace{\siggutter}\makebox[\sigcol][l]{#2}\par
}
