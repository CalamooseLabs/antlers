% Compatible packages
\usepackage{tocbasic}
\usepackage[english]{babel}
\usepackage{scrlayer-scrpage}
\usepackage{xcolor}
\usepackage{ulem}
\usepackage{hanging}
\usepackage{ifthen}
\usepackage[totpages]{zref}
\usepackage{tabularx}
\usepackage{array}
\usepackage{booktabs}
\usepackage{xltabular}

% Century Schoolbook (the typeface used by the U.S. Supreme Court).
\usepackage[T1]{fontenc}
\usepackage{tgschola}
\usepackage{xspace}

% Headings and title in the serif family (Century Schoolbook), not KOMA's default sans.
\addtokomafont{disposition}{\rmfamily}

% Document-wide constants.
\newcommand{\documentTitle}{Lease}
\newcommand{\omitText}{Intentionally Deleted}

% Defined-term machinery. \dtregister stores a term's display text; \dtuse renders it in
% small caps and links back to its definition; \dtdef marks the definition (the anchor).
\newcommand{\dtregister}[2]{\expandafter\def\csname dtdisplay@#1\endcsname{#2}}
\newcommand{\dtuse}[1]{\hyperlink{def:#1}{\textsc{\csname dtdisplay@#1\endcsname}}\xspace}
\newcommand{\dtdef}[1]{\hypertarget{def:#1}{\textsc{\csname dtdisplay@#1\endcsname}}\xspace}
\subimport{.}{defined_terms}

% Non-Section Header Style Function
\newcommand{\formaltext}[1]{%
	\begingroup
	\centering
	\normalfont\normalsize\bfseries #1\par
	\endgroup
	\vspace{0.5em}
}

\newcommand{\blankspace}{\ \newline \ }

% Exhibit heading: centered "Exhibit <letter>" + underlined uppercase title.
% Exhibits use this (not \section) so they stay out of the article numbering and TOC.
\newcommand{\exhibithead}[2]{%
	\begingroup
	\centering
	\normalfont\large\bfseries Exhibit #1\par
	\vspace{0.3em}
	\uline{\uppercase{#2}}\par
	\endgroup
	\vspace{1em}
}

% Completely redefine maketitle to avoid any errors
\makeatletter
\renewcommand{\maketitle}{%
	\begingroup
	\centering
	\normalfont\large\bfseries\color{black!80}%
	\@title\par
	\vspace{0.5em}
	\endgroup
}
\makeatother

% Safer section formatting
\makeatletter
\renewcommand{\sectionlinesformat}[4]{%
	\Ifstr{#1}{section}{%
		\begingroup
		\centering
		ARTICLE \Roman{section}\par
		\vspace{0.3em}%
		\uline{\uppercase{#4}}\par
		\endgroup
	}{%
		#3#4%
	}%
}
\makeatother

\RedeclareSectionCommand[
	beforeskip=2ex plus 1ex minus .2ex,
	afterskip=3ex plus .2ex,
	font=\normalsize\bfseries
]{section}

% Page styling
\clearpairofpagestyles
\cfoot{-- \thepage{} of \ztotpages --}

% Redefine subsection for legal document style
\makeatletter
\renewcommand\subsection{\@startsection{subsection}{2}{\z@}%
	{-3.25ex\@plus -1ex \@minus -.2ex}%
	{-1em}% Negative value makes it run-in
	{\normalfont\normalsize\bfseries}}
\makeatother

% Format the subsection number
\renewcommand{\thesubsection}{\arabic{section}.\arabic{subsection}}

% Create a wrapper command that adds underlining and period
\let\originalsubsection\subsection
\renewcommand{\subsection}[1]{%
	\originalsubsection{\uline{#1}.}%
}

% Redefine subsubsection with explicit paragraph shaping
\makeatletter
\renewcommand\subsubsection{\@startsection{subsubsection}{3}{\parindent}% Indented by \parindent
	{-3.25ex\@plus -1ex \@minus -.2ex}%
	{-1em}% Negative value makes it run-in
	{\normalfont\normalsize\bfseries}}
\makeatother

% Format the subsubsection number
\renewcommand{\thesubsubsection}{\arabic{section}.\arabic{subsection}.\arabic{subsubsection}}

% Create a wrapper command with explicit paragraph shaping
\newlength{\subsubindent}
\setlength{\subsubindent}{\parindent}

\let\originalsubsubsection\subsubsection
\renewcommand{\subsubsection}[1]{%
	\originalsubsubsection{\uline{#1}.}%
	\parshape 2 \subsubindent \dimexpr\linewidth-\subsubindent\relax \subsubindent \dimexpr\linewidth-\subsubindent\relax%
}

% % Define a new counter for subsubsubsection
\newcounter{subsubsubsection}[subsubsection]
\renewcommand{\thesubsubsubsection}{\arabic{section}.\arabic{subsection}.\arabic{subsubsection}.\arabic{subsubsubsection}}

\newcommand{\subsubsubsection}[1]{%
  \par\vspace{1ex}%
  \refstepcounter{subsubsubsection}% INCREMENT COUNTER FIRST
  \noindent\hspace{2\parindent}% Double indent
  \textbf{\thesubsubsubsection~\uline{#1}.}~\ignorespaces% Then use the number
  \parshape 2 2\subsubindent \dimexpr\linewidth-2\subsubindent\relax 2\subsubindent \dimexpr\linewidth-2\subsubindent\relax%
}

% Table of Content
% Change the TOC title
\addto\captionsenglish{%
  \renewcommand{\contentsname}{Table of Contents}%
}
% Change section numbering to Roman numerals
\renewcommand{\thesection}{\Roman{section}}
% Customize the TOC entry format for sections
\DeclareTOCStyleEntry[
    entrynumberformat=\articleformat,
    numwidth=8em
]{tocline}{section}
% Widen the numbered-entry boxes so 2-digit numbers don't collide with the titles.
\DeclareTOCStyleEntry[numwidth=3em, numsep=0.5em]{tocline}{subsection}
\DeclareTOCStyleEntry[numwidth=4em, numsep=0.5em]{tocline}{subsubsection}

% Define the custom format function
\newcommand{\articleformat}[1]{Article #1}
% Custom TOC command that bypasses the section formatting
\makeatletter
\renewcommand\tableofcontents{%
  \begingroup
  \setlength{\parindent}{0pt}
  \setlength{\parskip}{0pt}
  {\centering\Large\bfseries\contentsname\par}
  \vspace{1em}
  \@starttoc{toc}%
  \endgroup
}
\makeatother

% hyperref loads last: makes the TOC entries, cross-references, and defined-term uses
% clickable. hidelinks keeps them black/borderless (clean in print, still clickable).
\usepackage[hidelinks]{hyperref}
