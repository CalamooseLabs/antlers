...
- **`programs.vibe`** installs a configured `vibe` command. `flakes/vibe/package.nix` follows the **zed-editor wrapper pattern** — a `callPackage`-able *function* `cfg → writeShellApplication "vibe"` (exposed as `lib.<system>.mkVibeWrapper`; default-config derivation as `packages.<system>.vibe`). Interactively, `vibe` serialises the pinned `{ model, effortLevel, permissions, …extraSettings }` to a throwaway settings.json and runs `claude --settings …` (`VIBE_MODEL`/`VIBE_EFFORT` env-override via jq). `vibe --remote-control [name]` (or `VIBE_REMOTE_CONTROL=1`) instead runs `claude remote-control --name …` so the session is driven from claude.ai / mobile. **Caveat (verified against claude-code 2.1.170): `claude remote-control` accepts only `--name` and `--permission-mode` — not `--settings`/`--model`/`--effort`.** So in remote-control mode only `permissions.defaultMode` is delivered (as `--permission-mode`); model/effort are chosen client-side. Since `services.vibe` sessions default to remote-control, the `programs.vibe.model`/`effort` pins reach *interactive* `vibe` use, not the web-service sessions.

- **`services.vibe`** runs `vibe-server`, a Deno web service following the **lanserver pattern** exactly (`flakes/vibe-server/{package.nix,app/}`: `deno compile`, FOD deno-cache, denort zip, nix-ld; **zero external imports** so the FOD build stays offline — do not add `jsr:`/`npm:`/`https:`/`@std/*` imports). The web UI: shared-password login → HMAC-signed cookie; lists `services.vibe.directories`; spawns a `vibe` session per chosen dir (via the overridable `sessionCommand`, default `["${vibePackage}/bin/vibe" "--remote-control" "@NAME@"]`, `@DIR@`/`@NAME@` substituted, launched under `setsid` for clean process-group kill, output captured to `/var/lib/vibe/logs/`); lists/kills sessions; SSE-streams each log read-only. The browser is a **lifecycle manager** — you actually interact with a session through Claude Code Remote Control.

Unlike lanserver's strict sandbox, the unit uses a static `User`/`Group`, `StateDirectory=vibe`, `ReadWritePaths = ["/var/lib/vibe"] ++ directories`, and **auto-relaxes `ProtectHome`** (→ `false`, with a `warnings` notice) when any configured dir is under `/home`; never set `MemoryDenyWriteExecute` (breaks V8's JIT). The service user must be authenticated to Claude — via `environmentFile` (`ANTHROPIC_API_KEY=…`) and/or a pre-seeded `claudeConfigDir` (→ `CLAUDE_CONFIG_DIR`). `passwordFile`/`environmentFile` are read at runtime, never copied to the store. Note `templates/vibe-shell/` is unrelated (a dev-shell template, no `vibe` command).
